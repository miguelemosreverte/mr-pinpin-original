...
const elderMiniatures = Object.fromEntries([
  ['one-day-in-the-forest', 'papa-home'], ...elderParts.map(part => ['elder-' + part, part])
]);
...
test('all registered miniature files match dimensions and provenance hashes', () => {
  let legacy = 0, elder = 0;
  const exports = JSON.parse(readFileSync(join(root, 'images/published/elder-cycle/export-manifest.json'), 'utf8')).assets;
  for (const [id, cover] of Object.entries(registry.covers)) {
    const mini = cover.miniature;
    if (Object.hasOwn(elderMiniatures, id)) {
      elder++;
      const filename = `${elderMiniatures[id]}-miniature.webp`;
      assert.equal(mini.asset, `images/published/elder-cycle/${filename}`);
      assert.equal(mini.version, 1);
      const bytes = readFileSync(join(root, mini.asset));
      assert.equal(bytes.toString('ascii', 0, 4), 'RIFF');
      assert.equal(bytes.toString('ascii', 8, 12), 'WEBP');
      assert.equal(bytes.toString('ascii', 12, 16), 'VP8 ');
      assert.equal(bytes.readUInt16LE(26) & 0x3fff, mini.width);
      assert.equal(bytes.readUInt16LE(28) & 0x3fff, mini.height);
      const record = exports.find(asset => asset.target === filename);
      assert(record, `${id} export provenance`);
      assert.equal(createHash('sha256').update(bytes).digest('hex'), record.webSHA256);
  ...
  assert.equal(elder, 6);
});

test('Elder miniature whitelist rejects wrong paths and versions for all six editions', async () => {
  for (const id of Object.keys(elderMiniatures)) {
    for (const mutate of [
      mini => { mini.asset = '../outside.webp'; },
      mini => { mini.asset = 'https://example.com/miniature.webp'; },
      mini => { mini.asset = mini.asset.replace('.webp', '-other.webp'); },
      mini => { mini.asset = `images/covers/${id}/miniature/miniature-v1.png`; },
      mini => { mini.version = 2; }
    ]) {
      const data = structuredClone(registry);
      mutate(data.covers[id].miniature);
      const {titleCovers} = setup(data);
      await titleCovers.load();
      assert.equal(titleCovers.resolveMiniature(id, 'en'), null);
    }
  }
});
...
test('concurrent availability, miniatures and previews share successful resolution', async () => {
  ...
  assert.deepEqual(loaded.sort(), ['home-sweet-home', 'one-day-in-the-forest', 'timber-tractor']);
});
...
test('derivative dimensions and hashes retain the original source provenance', () => {
  ...
  for (const [id, cover] of Object.entries(registry.covers)) {
    const mini = cover.miniature;
    if (Object.hasOwn(elderMiniatures, id)) {
      assert.equal(mini.derivatives, undefined);
      continue;
    }
    checked++;
