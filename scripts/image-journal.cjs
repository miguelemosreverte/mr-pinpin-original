...
function failure(plan, id, startedAt, observedAt, error) {
  const shot = plan.shots.find(shot => shot.id === id);
  if (!shot) throw new Error('Unknown shot: ' + id);
  let attempt = 1;
  const stem = resolve(shot.output.replace(/\.png$/, ''));
  while (fs.existsSync(stem + '-failed-' + String(attempt).padStart(2,'0') + '.json')) attempt++;
  const file = stem + '-failed-' + String(attempt).padStart(2,'0');
  fs.mkdirSync(path.dirname(file), {recursive:true});
  const record = {...shot, status:'tool-error-no-output', tool:'built-in imagegen', startedAt, observedAt, error, result:null};
  json(file + '.json', record);
  fs.writeFileSync(file + '.md', `# Failed Image Request: ${shot.title}\n\n- Started UTC: ${startedAt}\n- Failure observed UTC: ${observedAt}\n- Error: ${error}\n- No image was produced.\n- Intended output: ${shot.output}\n\n## Exact Prompt\n\n\`\`\`text\n${shot.prompt}\n\`\`\`\n\n## References\n\n${shot.references.map(ref => '- '+ref.path+': '+ref.role).join('\n')}\n`);
  return record;
}
...
function build(plan) {
  ...
  const failures = plan.shots.flatMap(shot => {
    const directory = path.dirname(resolve(shot.output));
    if (!fs.existsSync(directory)) return [];
    const prefix = path.basename(shot.output, '.png') + '-failed-';
    return fs.readdirSync(directory).filter(name => name.startsWith(prefix) && name.endsWith('.json'))
      .map(name => ({...read(path.join(directory,name)), file:path.join(directory,name)}));
...
if (require.main === module) {
  ...
  else if (command === 'failure') failure(plan, id, ...args);
