...
Estimated linear luminance / bright specular-candidate weight from painted SDR color. Bright diffuse paint and reflections cannot be separated; not emission, light-source segmentation, radiance or HDR.

GRAY8 = round(255 * (0.2126 Rlinear + 0.7152 Glinear + 0.0722 Blinear)); inverse-sRGB transfer applied to original RGB only. Sample as linear UNORM.
...
Full pass: 1536x1024. Preview: 768x512. Sidecar JSON records settings, encoders, output hashes and compact field when applicable. No original input is modified. Timestamp is retained for identical outputs; SOURCE_DATE_EPOCH sets it on new builds.
