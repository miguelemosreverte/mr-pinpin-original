...
const target=chapter==='academy'?'academy-r02-21':'elder-r4-091';
(async()=>{
 const browser=await chromium.launch({channel:'chrome',headless:true});
 const page=await browser.newPage({viewport:{width:1440,height:1000}});
 await page.goto(`http://127.0.0.1:8782/storyboard/review/chapter-workshop.html?revision=4&chapter=${chapter}&lang=ru&view=production`);
