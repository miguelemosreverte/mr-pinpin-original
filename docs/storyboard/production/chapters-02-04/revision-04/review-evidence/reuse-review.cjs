...
const prior=JSON.parse(fs.readFileSync(path.join(repo,'docs/storyboard/production/chapters-02-04/revision-03/elder/chapter.json')));
const plan=JSON.parse(fs.readFileSync(path.join(repo,'docs/storyboard/production/chapters-02-04/revision-04/elder/shot-plan.json')));
const manifest={scenes:plan.scenes.filter(s=>s.assetMode==='reuse').map(s=>({...prior.scenes.find(a=>a.id===s.r3Scene),id:s.id,text:{en:[s.dialogue_en]}}))};
...
(async()=>{
 ...
 for(let start=Number(process.env.START||0);start<Math.min(manifest.scenes.length,Number(process.env.STOP||999));start+=12){
  ...
  await p.screenshot({path:path.join(__dirname,`${chapter}-reuse-review-${start+1}.png`),fullPage:true});
 }
