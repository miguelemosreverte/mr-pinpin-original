...
(async()=>{
 ...
 for(const chapter of (process.env.CHAPTER?[process.env.CHAPTER]:['elder'])){
  ...
  for(const width of [1440,390])for(const lang of ['ru','en','es'])for(const view of ['reading','preproduction','production']){
   ...
   await page.goto(`${base}?revision=5&chapter=${chapter}&lang=${lang}&view=${view}`);
   ...
   const check=await page.evaluate(()=>({lang:document.documentElement.lang,overflow:document.documentElement.scrollWidth>innerWidth,images:document.images.length,broken:[...document.images].filter(i=>!i.naturalWidth).map(i=>i.src),pending:document.querySelectorAll('.picture-frame.pending').length,ids:[...document.querySelectorAll('[data-scene]')].map(e=>e.dataset.scene),distorted:[...document.images].filter(i=>{const r=i.getBoundingClientRect();return r.width>0&&r.height>0&&Math.abs(r.width/r.height-i.naturalWidth/i.naturalHeight)>.015;}).map(i=>i.src),metadata:document.querySelectorAll('.production-notes').length,cameraRecords:document.querySelectorAll('.camera-transform').length}));
   const issues=[];
   ...
   if(view!=='production'&&check.cameraRecords)issues.push('camera metadata outside production');
   if(view==='production'&&check.cameraRecords!==35)issues.push('camera records missing');
