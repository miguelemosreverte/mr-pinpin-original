...
a=argparse.ArgumentParser();a.add_argument('--root',required=True);a.add_argument('--stop',type=int,required=True);a.add_argument('--stage',choices=['base','rear','final'],required=True);a.add_argument('--source',nargs='+',required=True);a.add_argument('--base-version',default='v1',choices=['v1','v2']);args=a.parse_args()
assert args.stop in {1,2,3,5,10,11,12}
...
basefile=f'panorama-{version}.png';fitdir=f'fit-{version}'
def record(kind,src,inputfiles,roles,camera=None):
 image=s/f'{kind}-v1.png';prompt=s/f'{kind}-prompt.txt'
 if kind=='panorama':image=s/basefile;prompt=s/('panorama-prompt.txt' if version=='v1' else f'panorama-{version}-prompt.txt')
 ...
 (s/(f'panorama-{version}-generation.json' if kind=='panorama' and version!='v1' else f'{kind}-generation.json')).write_text(json.dumps(r,indent=2)+'\n')
def stack(keys,name):
 r={'schemaVersion':1,'id':f'stop{args.stop:02d}-{name}','inputs':{k:{'file':v,'sha256':sha(s/v)} for k,v in [('base',basefile)]+[(k,f'{k}-v1.png') for k in keys]}}
...
if args.stage=='base':
 display=(s/'display-guide65-v1/guide-equirect.png').exists()
 frame=s/('source-display.png' if display else 'source.png')
 guide=s/('display-guide65-v1/guide-equirect.png' if display else 'raw-guide65-v1/guide-equirect.png')
 record('panorama',args.source[0],[guide,frame]+([b/'pilot/fov-correction/accepted-reference.png'] if version=='v1' else []),['literal projected edit target65degree','browser-decoded source camera/material/light authority' if display else 'raw source camera/material/light authority','sphere continuity only'])
 subprocess.run([py,str(b/'fit_camera.py'),'--reference',str(frame),'--panorama',str(s/basefile),'--out',str(s/fitdir)],check=True)
 ...
 c=json.loads((s/fitdir/'fit.json').read_text())['camera'];camera={'yaw':c['yawRadians'],'pitch':c['pitchRadians'],'fov':c['verticalFovDegrees']}
 ...
 if version!='v1':r['prompt']=pre+f'panorama-{version}-prompt.txt';r['record']=pre+f'panorama-{version}-generation.json'
