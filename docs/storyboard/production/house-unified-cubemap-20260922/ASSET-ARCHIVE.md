...
Initial C catalog: **531 production assets** (800,531,775 bytes), **286 archive assets** (943,485,561 bytes), 817 total. Production selection makes v2 eligible for the review release; it does not imply a final artistic approval.
...
## Grayscale comparison addition

A lossless grayscale runtime derivative adds one production asset without new archive originals. Final totals are **532 production / 286 archive**. All 817 earlier entries are unchanged. See [GRAY-RUNTIME.md](GRAY-RUNTIME.md) for decoded-pixel equality, exact hashes and the new release recipe.
