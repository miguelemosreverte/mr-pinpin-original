...
const rooms={
 ...
 legacy:{label:'Legacy illustrated room',atlas:'storyboard/images/house-menu/room-unified-cubemap-v2.webp',role:'Legacy illustration · style experiment from an older, different house model. This is not the geometry shown in the other three rooms.'}
};
...
async function loadRoom(id){
 if(!rooms[id])id='common';selected=id;const token=++request,room=rooms[id];
 limits.yaw=room.yaw||0;limits.pitch=room.pitch||0;
 renderer?.destroy();renderer=null;state('loading','Loading '+room.label.toLowerCase()+'…');
 $('room').value=id;$('view-role').textContent=room.role;
 fallback.src=room.atlas;fallback.alt=room.label+' flat atlas: front, right, back / left, up, down.';
 const link=$('atlas-download');link.href=room.atlas;link.download=room.atlas.split('/').pop();
 $('view-evidence').href=id==='legacy'?'storyboard/locations/pinpin-house/references.json':room.atlas.replace('cube-atlas.webp','review-manifest.json');
 ...
 try{
  ...
  renderer=unifiedRenderer(canvas,image);reset();draw(controls.snapshot);state('active','');
...
Object.defineProperty(stage,'workshopView',{value:Object.freeze({get snapshot(){return{room:selected,backend,asset:rooms[selected].atlas,draws,...controls.snapshot};}})});
