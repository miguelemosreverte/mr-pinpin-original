...
function schedule(){if(!['panorama','cubemap','cubemap-unified','cubemap-gray'].includes(viewport.dataset.view)&&!frame&&ready&&!reduced.matches&&!failed)frame=requestAnimationFrame(render);}
...
async function load(){
 const token=++version;
 if(['panorama','cubemap','cubemap-unified','cubemap-gray'].includes(viewport.dataset.view)){flat('panorama-paused');return;}
 ...
 try{
  ...
  if(token!==version||reduced.matches||['panorama','cubemap','cubemap-unified','cubemap-gray'].includes(viewport.dataset.view))return;
...
viewport.addEventListener('roomviewchange',()=>{if(['panorama','cubemap','cubemap-unified','cubemap-gray'].includes(viewport.dataset.view)){version++;flat('panorama-paused');}else {geometryDirty=true;load();}});
load();
