import {WIDTH,HEIGHT,COLS,ROWS,createMesh,projectMesh,projectPoint} from './home-depth-geometry.js';
import {createRenderer} from './home-depth-gl.js';
...
const contours=[...room.querySelectorAll('.contour')].map(path=>{
 ...
 for(let d=0;d<length;d+=3){const p=path.getPointAtLength(d);points.push([p.x,p.y]);}
...
viewport.depthView=Object.freeze({
 get snapshot(){return {backend:viewport.dataset.depth,convention:'white-near',focus,target,selected:selected?.path.id||null,draws,maxBlurCSS:3.2,camera:{...viewport.cameraState}};},
...
function schedule(){if(!frame&&ready&&!reduced.matches&&!failed)frame=requestAnimationFrame(render);}
...
async function load(){
 ...
 try{
  await art.decode();const depth=new Image();depth.src='storyboard/images/house-menu/room-depth-v1.webp';await depth.decode();
