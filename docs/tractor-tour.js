...
import {tourGestures} from './tractor-tour-gestures.js?v=tour-source-lock-v5';
...
let camera={yaw:0,pitch:0,fov:75,width:1,height:1},wheelQuietUntil=0,lastWheelAt=0;
...
function seek(p){if(!ready)return;lastInteractionAt=performance.now();video.pause();desired=Math.min(((p%1)+1)%1*video.duration,video.duration-.001);if(!seekFrame)seekFrame=requestAnimationFrame(pump);labels();}
...
function finishOrbit(p=null){epoch++;prefetch.cancel();bridge.cancel();mode='orbit';wheelQuietUntil=0;snap=null;video.pause();if(p!==null)seek(p);drawOrbit();labels();warmNearby();status('One finger or left drag moves around. Two fingers, touchpad scroll or right drag looks around.');}
function orbit(p=null){if(['recentering','bridge-out'].includes(mode)){if(p!==null)pendingOrbit=p;return exitPromise;}if(['look','bridge-in'].includes(mode)&&(sourceLock||active?.bridge)&&!reduced()){pendingOrbit=p;exitPromise=leaveLook();return exitPromise;}finishOrbit(p);return Promise.resolve();}
...
async function look(from=null,forced=null){
  ...
  prefetch.cancel();video.pause();active=stop;camera={...stop.face,width:stage.clientWidth,height:stage.clientHeight};mode='snapping';$('stop').value=stop.id;labels();status('Moving along the shortest route to the available viewpoint. The opening view stays fixed. Finish this gesture, then move again to look around.');
  try{await Promise.all([snapTo(stop,own,origin),loadRenderer(stop,own)]);if(own!==epoch)return;if(sourceLock)renderer.setSource(video);if(!sourceLock&&stop.bridge&&!reduced()){await bridge.play(stop.bridge.in,{valid:()=>own===epoch,onReady:()=>{mode='bridge-in';labels();status('Entering the panorama through its generated camera transition.');}});if(own!==epoch)return;}mode='look';warmNearby(stop);wheelQuietUntil=Math.max(performance.now()+120,lastWheelAt+160);draw();labels();status('Facing the tractor. A new gesture looks around; left drag returns to orbit.');}catch{if(own!==epoch)return;orbit();status('The panorama could not load. Orbit remains available.');}
}
function head(yaw,pitch,zoom=0){if(mode!=='look')return;lastInteractionAt=performance.now();camera.yaw=wrap(camera.yaw+(Number.isFinite(yaw)?yaw:0));camera.pitch=clamp(camera.pitch+(Number.isFinite(pitch)?pitch:0),-89*RAD,89*RAD);camera.fov=clamp(camera.fov*Math.exp((Number.isFinite(zoom)?zoom:0)*.0025),25,110);requestDraw();}
function wheelInput(dx,dy,ctrl){const now=performance.now();lastWheelAt=now;if(mode==='orbit'){look();return;}if(mode!=='look'||now<wheelQuietUntil){wheelQuietUntil=now+160;return;}if(ctrl)head(0,0,dy);else head(dx*.003,-dy*.003);}
const gestures=tourGestures(stage,{progress,orbit,isLooking:()=>mode==='look',startLook:look,drag:(dx,dy)=>head(-dx/stage.clientHeight*camera.fov*RAD,dy/stage.clientHeight*camera.fov*RAD),wheelInput,zoom:delta=>head(0,0,delta)});
$('look').addEventListener('click',()=>look());$('orbit').addEventListener('click',()=>orbit());$('face').addEventListener('click',()=>{if(mode==='orbit')look();else if(active){camera={...active.face,width:stage.clientWidth,height:stage.clientHeight};draw();}});
...
canvas.addEventListener('webglcontextlost',e=>{e.preventDefault();prefetch.cancel();renderer?.destroy();renderer=null;renderedStop=null;images.clear();finishOrbit();prefetch.cancel();status('Panorama rendering paused. Orbit remains available.');});
Object.defineProperty(window,'tractorTour',{value:Object.freeze({get snapshot(){return{ready,sourceLock,displayedMediaTime,displayedFrame,mode,stop:active?.id||null,time:video.currentTime,progress:progress(),seeking:video.seeking,queuedSeek:desired,snap,wheelQuietUntil,bridgeTime:bridge.time,pendingOrbit,available:available.map(s=>s.id),camera:{...camera},draws,performance:{images:images.stats,panorama:renderer?.stats??null,orbit:flatOrbit?.stats??null},gestures:gestures.state};},setCamera(c){if(mode==='look'){lastInteractionAt=performance.now();Object.assign(camera,c);camera.pitch=clamp(camera.pitch,-89*RAD,89*RAD);draw();}}})});
