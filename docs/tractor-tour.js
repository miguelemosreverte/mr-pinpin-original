...
import {tourGestures} from './tractor-tour-gestures.js?v=tour-controls-v1';
...
function finishOrbit(p=null){epoch++;prefetch.cancel();bridge.cancel();mode='orbit';resetLookInput();snap=null;video.pause();if(p!==null)seek(p);drawOrbit();labels();queueOrbitWarm();status('Move the pointer or use one finger to orbit. Two-finger scroll or right drag looks around; keep moving through the transition.');}
...
function wheelInput(dx,dy,ctrl){if(!dx&&!dy)return;if(['orbit','recentering','bridge-out'].includes(mode))look();if(ctrl)head(0,0,dy);else head(dx*.003,-dy*.003);}
