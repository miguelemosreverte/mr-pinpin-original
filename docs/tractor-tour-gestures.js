...
export function tourGestures(stage,api){
  ...
  stage.addEventListener('wheel',e=>{e.preventDefault();const unit=e.deltaMode===1?16:e.deltaMode===2?stage.clientHeight:1;api.wheelInput(e.deltaX*unit,e.deltaY*unit,e.ctrlKey);},{passive:false});
  stage.addEventListener('pointerdown',e=>{
    if(e.pointerType==='mouse'&&e.button!==0&&e.button!==2)return;
    points.set(e.pointerId,{x:e.clientX,y:e.clientY});stage.setPointerCapture(e.pointerId);
    if(e.pointerType==='touch'&&points.size===2){const origin=gesture?.origin??api.progress();gesture={kind:'two',last:center(),origin,accepted:api.isLooking()};api.startLook(origin);return;}
    if(points.size>1)return;
    const right=e.pointerType==='mouse'&&e.button===2;
    gesture={kind:right?'right':'pending',accepted:api.isLooking(),origin:api.progress(),x:e.clientX,y:e.clientY,lastX:e.clientX,lastY:e.clientY,time:performance.now(),touch:e.pointerType==='touch'};
  ...
  stage.addEventListener('pointermove',e=>{
    ...
    if(gesture.kind==='two'){if(points.size<2)return;const c=center();if(gesture.accepted){api.drag(c.x-gesture.last.x,c.y-gesture.last.y);if(c.d>0&&gesture.last.d>0)api.zoom(Math.log(gesture.last.d/c.d)*400);}gesture.last=c;return;}
    if(gesture.kind==='right'){if(gesture.accepted)api.drag(e.clientX-gesture.lastX,e.clientY-gesture.lastY);gesture.lastX=e.clientX;gesture.lastY=e.clientY;return;}
  ...
  return {get state(){return{kind:gesture?.kind||'idle',pointers:points.size,accepted:gesture?.accepted||false};}};
}
